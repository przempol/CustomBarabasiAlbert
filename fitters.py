...
def solution_me(k: int, alpha: float, m: int = 1) -> float:
    ...
    if alpha > 0.001:
        ...
        m = 1
        ret = 2 / (2*m + 2 - alpha * m)
        ret *= gamma(2 * m / alpha - m + 1 + 2/alpha)
        ret /= gamma(2 * m / alpha - m)
        ret *= gamma(2 * m / alpha - 2 * m + k)
...
def mlm_fit(vertex_degrees: list, x_min: int) -> (float, float):
    ...
    vertex_degrees = vertex_degrees[vertex_degrees >= 1]
    ...
    print(f'Best fit, using x_min = {x_min} is alpha = {alpha} +- {sigma}')
    return alpha, sigma
